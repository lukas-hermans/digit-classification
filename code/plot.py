...
for deg in deg_list:
    ...
    plt.ylabel(r"binary training error $\hat{\ell}_{D^{(5)}}$", size=size)
    plt.tight_layout()
...
for deg in deg_list:
    ...
    plt.plot(n_epoch, test_error,
             label=r"final: $\vec{\alpha}_{fin}$",
             color="r", linestyle="solid")
    plt.plot(n_epoch, test_error_min,
             label=r"minimizing: $\vec{\alpha}_{min}$",
             color="b", linestyle="dotted")
    plt.plot(n_epoch, test_error_avg,
             label=r"average: $\langle\vec{\alpha}\rangle$",
             color="green", linestyle="dashed")
    ...
    plt.xticks(n_epoch)
    plt.xlabel(r"$N_{epoch}$", size=size)
    plt.ylabel(r"binary test error $\hat{\ell}_{D^{(5)}}$", size=size)
    plt.tight_layout()
...
#     training_error.append(data[0])
#     training_error_min.append(data[1])
#     training_error_avg.append(data[2])

#     test_error.append(data[3])
#     test_error_min.append(data[4])
#     test_error_avg.append(data[5])
...
#              color="r", linestyle="solid")
#     plt.vlines(n_epoch[ind_list[i]], 0, 1.5*np.max(data[3]),
#                color="r", linestyle="solid")
#     plt.plot(
#         n_epoch, data[4], label=r"minimizing: $\vec{\alpha}_{min}$",
#         color="b", linestyle="dotted")
#     plt.vlines(n_epoch[ind_min_list[i]], 0, 1.5*np.max(data[4]),
#                color="b", linestyle="dotted")
#     plt.plot(
#         n_epoch, data[5], label=r"average: $\langle\vec{\alpha}\rangle$",
#         color="green", linestyle="dashed")
#     plt.vlines(n_epoch[ind_avg_list[i]], 0, 1.5*np.max(data[5]),
#                color="green", linestyle="dashed")
